...
//   props: ["config", "translations"],
  setup(props) {
    ...
    try {
      ...
      let test = deepCopy(defaultUserConfigOptions)
    ...
    ipcRenderer.send(IpcChannels.remoteLog, "🦄 (-99)")


    // const initialConfig = new ElectronStoreConfigRepository(deepCopy(defaultUserConfigOptions)).getConfig();

    // ipcRenderer.send(IpcChannels.remoteLog, "🦄 (-99): " + initialConfig);

    // const config = ref(initialConfig);
    // const translations = ref(getTranslationSet(initialConfig.generalOptions.language));
    ...
    // // Implement the onErrorCaptured lifecycle hook
