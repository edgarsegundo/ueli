...
import { GrammarCheckerOptions } from "../../../common/config/grammar-checker-options";

export class GrammarCheckerPlugin implements ExecutionPlugin {
    public readonly pluginType = PluginType.GrammarCheckerPlugin;
    private config: GrammarCheckerOptions;
    private delay: NodeJS.Timeout | number | undefined;
    private readonly clipboardCopier: (value: string) => Promise<void>;

    constructor(config: GrammarCheckerOptions, clipboardCopier: (value: string) => Promise<void>) {
        this.config = config;
    ...
    public updateConfig(updatedConfig: UserConfigOptions): Promise<void> {
        return new Promise((resolve) => {
            this.config = updatedConfig.grammarCheckerOptions;
            resolve();
        });
    }
